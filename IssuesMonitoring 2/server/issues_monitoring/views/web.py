...
@app.route('/registrar-presenca', methods=['POST'])
def presenca_post():
    evento = request.form.get('evento') or ''
    laboratorio =  request.form.get('lab_id') or ''

    if not autenticado():
        return redirect(url_for('login'))

    if '' in [evento, laboratorio]:
...
def login_post():
    ...
    try:
        if '' in senha:
            if controllers.autenticar_usuario_lab(usuario):
                session['id'] = usuario
...
def laboratorios():
    ...
    return render_response('labs.html',
